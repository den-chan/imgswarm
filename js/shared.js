...
self.onconnect = function(mp) {
  port = mp.ports[0]
  port.onmessage = function (e) {
    function postBack (result) {
      return port.postMessage({messageType: data.messageType, value: result})
    }
    function errorBack (result) {
      ...
      return port.postMessage({messageType: data.messageType, error: error})
    }
    var data = e.data;
    switch (data.messageType) {
      ...
      case "checkunique":
        ...
        userShared = new User(authObj);
        return userShared.checkunique().then(postBack).catch(errorBack)
        
      case "createuser":
        return userShared.create(data.value).then(userShared.login).then(postBack).catch(errorBack);
        
      case "login":
        var authObj = new Auth(data.value);
        userShared = new User(authObj);
        return userShared.login().then(postBack).catch(errorBack)
        
      case "logout":
        return userShared.logout().then(postBack)
...
function User (auth) {
  ...
  this.logout = function () { return Promise.resolve((userShared = null) || "success") }
